...
if (!existsSync(distDir)) {
  console.error('✖️  Prerender failed: dist directory not found. Run `npm run build` first.');
  process.exit(1);
}
...
const buildHtml = (template: string, meta: RouteMeta): string => {
  ...
  const fullTitle = meta.title === SEO_CONFIG.siteName
    ? meta.title
    : `${meta.title} | ${SEO_CONFIG.siteName}`;
  const fullUrl = `${SEO_CONFIG.baseUrl}${meta.url}`;
  const fullImage = image.startsWith('http') || image.startsWith('data:')
    ? image
  ...
  html = replaceTag(html, /<meta property="og:title" content="[^"]*"\s*\/?>/, `<meta property="og:title" content="${escapeHtml(fullTitle)}" />`);
  html = replaceTag(html, /<meta property="og:description" content="[^"]*"\s*\/?>/, `<meta property="og:description" content="${escapeHtml(description)}" />`);
  html = replaceTag(html, /<meta property="og:type" content="[^"]*"\s*\/?>/, `<meta property="og:type" content="${escapeHtml(ogType)}" />`);
  html = replaceTag(html, /<meta property="og:url" content="[^"]*"\s*\/?>/, `<meta property="og:url" content="${escapeHtml(fullUrl)}" />`);
  html = replaceTag(html, /<meta property="og:image" content="[^"]*"\s*\/?>/, `<meta property="og:image" content="${escapeHtml(fullImage)}" />`);

  if (html.includes('meta property="og:image:alt"')) {
    html = replaceTag(html, /<meta property="og:image:alt" content="[^"]*"\s*\/?>/, `<meta property="og:image:alt" content="${escapeHtml(meta.title)}" />`);
  } else {
    html = insertAfter(html, /<meta property="og:image"[^>]*>/, `\n    <meta property="og:image:alt" content="${escapeHtml(meta.title)}" />`);
  }

  html = replaceTag(html, /<meta name="twitter:title" content="[^"]*"\s*\/?>/, `<meta name="twitter:title" content="${escapeHtml(fullTitle)}" />`);
  html = replaceTag(html, /<meta name="twitter:description" content="[^"]*"\s*\/?>/, `<meta name="twitter:description" content="${escapeHtml(description)}" />`);
  html = replaceTag(html, /<meta name="twitter:image" content="[^"]*"\s*\/?>/, `<meta name="twitter:image" content="${escapeHtml(fullImage)}" />`);

  if (html.includes('meta name="twitter:image:alt"')) {
    html = replaceTag(html, /<meta name="twitter:image:alt" content="[^"]*"\s*\/?>/, `<meta name="twitter:image:alt" content="${escapeHtml(meta.title)}" />`);
  } else {
    html = insertAfter(html, /<meta name="twitter:image"[^>]*>/, `\n    <meta name="twitter:image:alt" content="${escapeHtml(meta.title)}" />`);
  }

  if (html.includes('rel="canonical"')) {
    html = replaceTag(html, /<link rel="canonical" href="[^"]*"\s*\/?>/, `<link rel="canonical" href="${escapeHtml(fullUrl)}" />`);
  } else {
    html = insertAfter(html, /<meta name="author"[^>]*>/, `\n    <link rel="canonical" href="${escapeHtml(fullUrl)}" />`);
  }

 const articleMeta = meta.type === 'article' && meta.article
    ? ['author', 'publishedTime', 'modifiedTime', 'section', 'tag']
        .flatMap((key) => {
          const value = meta.article?.[key as keyof typeof meta.article];
          if (!value) return [];
          if (Array.isArray(value)) {
            return value.map((item) => `<meta property="article:${key.replace(/([A-Z])/g, '-$1').toLowerCase()}" content="${escapeHtml(item)}" />`);
          }
          return [`<meta property="article:${key.replace(/([A-Z])/g, '-$1').toLowerCase()}" content="${escapeHtml(value)}" />`];
        })
    : [];
